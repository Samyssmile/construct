...
export const Playground = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const nav = canvasElement.querySelector('aside nav[aria-label]');
    ...
    <aside class="ct-sidebar ct-sidebar--side" data-state="open" aria-label="Folder navigation">
      <div class="ct-sidebar__header">
        ...
        <button class="ct-button ct-button--ghost ct-button--icon ct-button--sm" type="button" aria-label="Create folder">
          <span class="ct-icon ct-icon--sm"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span>
        </button>
      </div>
      <div class="ct-sidebar__content">
        <nav aria-label="Folders">
          <ul class="ct-nav-list">
            <li><a class="ct-nav-item ct-nav-item--active" href="#" aria-current="page">
              <span class="ct-nav-item__icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></span>
...
`,
  play: async ({ canvasElement }) => {
    ...
    // Nav list lives inside a labelled navigation landmark
    const nav = canvas.getByRole('navigation', { name: 'Folders' });
    expect(nav).toBeInTheDocument();
    ...
    <aside class="ct-sidebar ct-sidebar--over" data-state="open" aria-label="Navigation" style="position: absolute;">
      <div class="ct-sidebar__header">
        ...
        <button class="ct-button ct-button--ghost ct-button--icon ct-button--sm" type="button" aria-label="Close menu">
          <span class="ct-icon ct-icon--sm"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></span>
        </button>
      </div>
      <div class="ct-sidebar__content">
...
`,
  play: async ({ canvasElement }) => {
    ...
    // Nav list lives inside a labelled navigation landmark
    const nav = canvas.getByRole('navigation', { name: 'Main menu' });
    expect(nav).toBeInTheDocument();
